from pgtkb import (
    ...
    pressed_bg, ImagePack, Uplclick, Audio, Downlclick
)
import pygame
...
@startbutton.on_event(Uplclick)
def play_happybirthday():
    mainapp.screen.blank_newbg(flower.surface)
    baihat.set_volume(0.5)
    baihat.play()
    event_manage = mainapp.event_manager
    event_manage.add_kmod_func((pygame.KMOD_CTRL, pygame.KMOD_ALT),func= baihat.pause)
    event_manage.add_kmod_func((pygame.KMOD_CTRL,),pygame.K_r ,baihat.resume)
    event_manage.add_kmod_func((pygame.KMOD_CTRL,),pygame.K_a ,baihat.play)
    event_manage.add_kmod_func((pygame.KMOD_CTRL,),pygame.K_s ,baihat.stop)
mainapp.threadstart()